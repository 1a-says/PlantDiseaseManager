...
from orange_classifier_common import (
    ...
    plot_confusion_matrix, fix_corrupted_images, logger
)
...
def download_dataset(config):
    ...
    if os.path.exists(local_dataset_path):
        logger.info(f"找到本地数据集: {local_dataset_path}")
        
        # 检查数据集结构
        classes = [d for d in os.listdir(local_dataset_path) 
                  if os.path.isdir(os.path.join(local_dataset_path, d))]
        
        if not classes:
            logger.error(f"在 {local_dataset_path} 中找不到任何类别目录")
            return None
        ...
        total_images = 0
        for cls in classes:
            cls_dir = os.path.join(local_dataset_path, cls)
        ...
        logger.error(f"未找到本地数据集目录: {local_dataset_path}")
...
def process_dataset(download_path, output_path, config):
    ...
    logger.info(f"源路径: {download_path}")
    logger.info(f"输出路径: {output_path}")
    
    # 检查源路径是否存在
    if not os.path.exists(download_path):
        logger.error(f"源路径不存在: {download_path}")
        return False
...
def train():
    ...
    try:
        ...
        parser.add_argument('--fix_images', action='store_true', help='修复损坏的图像文件')
        ...
        # 图像修复（如果需要）
        if args.fix_images:
            logger.info("开始修复损坏的图像文件...")
            local_dataset_path = "plant_dataset"
            if os.path.exists(local_dataset_path):
                backup_dir = os.path.join(config.output_dir, "image_backup")
                fix_corrupted_images(local_dataset_path, backup_dir)
                logger.info("图像修复完成")
            else:
                logger.error(f"未找到数据集目录: {local_dataset_path}")
                return
        ...
        scheduler = optim.lr_scheduler.ReduceLROnPlateau(
            optimizer, mode='min', factor=0.1, patience=3  # 移除 verbose=True
        )
        ...
        config.save_metrics(basic_metrics, config_dict=config_dict)
