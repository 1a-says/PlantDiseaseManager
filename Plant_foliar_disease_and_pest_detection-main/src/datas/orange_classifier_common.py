...
class Config:
    def __init__(self):
        ...
        self.img_size = 224
        self.dataset_name = "plant_dataset"  # 使用本地植物病害数据集
        ...
        for k, v in metrics.items():
            if isinstance(v, (int, float)):
                ...
                try:
                    ...
                    logger.warning(f"跳过无法转换为数值的指标: {k} = {v}")
                    continue
...
class ValidatedImageFolder(Dataset):
    ...
    def __init__(self, root_dir, transform=None, class_to_idx=None):
        ...
        # 扫描并验证图像文件
        self._scan_and_validate_images()
        
        logger.info(f"数据集扫描完成: {len(self.valid_img_paths)} 张有效图像")
    
    def _scan_and_validate_images(self):
        """扫描目录并验证图像文件完整性"""
        if not os.path.exists(self.root_dir):
            logger.error(f"数据集目录不存在: {self.root_dir}")
            return
        
        # 获取所有类别目录
        class_dirs = [d for d in os.listdir(self.root_dir) 
                     if os.path.isdir(os.path.join(self.root_dir, d))]
        class_dirs.sort()  # 确保类别顺序一致
        
        self.classes = class_dirs
        if not self.class_to_idx:
            self.class_to_idx = {cls_name: idx for idx, cls_name in enumerate(class_dirs)}
        
        logger.info(f"发现类别: {self.classes}")
        ...
        for class_name in class_dirs:
            ...
            logger.info(f"类别 '{class_name}' 发现 {len(img_files)} 个图像文件")
            ...
            for img_path in img_files:
                ...
                try:
                    ...
                    # 重新打开图像以确保可以正常读取
                    with Image.open(img_path) as img:
                        img.convert('RGB')  # 确保可以转换为RGB
                    
                    # 图像有效，添加到样本列表
                    self.valid_img_paths.append((str(img_path), class_idx))
                    valid_files += 1
                    
                except (IOError, UnidentifiedImageError, OSError, Exception) as e:
                    corrupted_files.append((str(img_path), str(e)))
                    logger.warning(f"损坏图像已跳过: {img_path}, 错误: {str(e)}")
        
        logger.info(f"图像验证完成: 总计 {total_files} 个文件, 有效 {valid_files} 个, 损坏 {len(corrupted_files)} 个")
        
        if corrupted_files:
            logger.warning(f"发现 {len(corrupted_files)} 个损坏的图像文件:")
            for file_path, error in corrupted_files[:10]:  # 只显示前10个
                logger.warning(f"  - {file_path}: {error}")
            if len(corrupted_files) > 10:
                logger.warning(f"  ... 还有 {len(corrupted_files) - 10} 个损坏文件")
    
    def __len__(self):
        return len(self.valid_img_paths)
    
    def __getitem__(self, idx):
        img_path, label = self.valid_img_paths[idx]
        ...
        try:
            ...
            logger.error(f"加载图像时出错: {img_path}, 错误: {str(e)}")
            # 返回一个默认图像（黑色图像）
...
# 图像格式转换和修复工具
def fix_corrupted_images(img_dir, backup_dir=None):
    """批量修复损坏的图像文件"""
    ...
    for img_path in img_files:
        try:
            ...
            with Image.open(img_path) as img:
                ...
                fixed_count += 1
                logger.info(f"修复成功: {img_path.name}")
                
        except Exception as e:
            failed_count += 1
            logger.error(f"修复失败: {img_path.name}, 错误: {str(e)}")
    
    logger.info(f"图像修复完成: 成功 {fixed_count} 个, 失败 {failed_count} 个")
...
def load_data(data_path, config):
    ...
    try:
        ...
        train_dataset = ValidatedImageFolder(train_dir, transform=train_transform)
        
        logger.info(f"创建验证数据集: {val_dir}")
        val_dataset = ValidatedImageFolder(val_dir, transform=eval_transform, class_to_idx=train_dataset.class_to_idx)
        
        logger.info(f"创建测试数据集: {test_dir}")
        test_dataset = ValidatedImageFolder(test_dir, transform=eval_transform, class_to_idx=train_dataset.class_to_idx)
